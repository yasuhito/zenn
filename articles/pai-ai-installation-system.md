...
[PAI (Personal AI Infrastructure)](https://github.com/danielmiessler/PAI) は、Daniel Miessler が公開している「パーソナル AI スタック」です。Claude Code などの AI エージェントに、記憶・スキル・フックなどの機能を追加できます。

PAI の面白い特徴の一つが **「AI によるインストール」** です。人間がコマンドを一つずつ打つのではなく、AI エージェントに「このパックをインストールして」と頼むと、AI が対話形式でインストールを進めてくれます。

この記事では、PAI の AI インストールの仕組みを解説します。

## なぜ AI インストールなのか

従来のインストール方法には問題がありました：

### 問題 1: トークン制限

大きなパックは単一ファイルだと **25,000 トークンを超える** ことがあります。マークダウン内にすべてのコードを埋め込むと、AI が一度に処理できない量になってしまいます。

### 問題 2: AI による「改善」

マークダウン内のコードブロックを AI に読ませると、親切心から **コードを「簡略化」してしまう** ことがあります。268行の完全な実装が、50行の「同等品」になってしまうのです。

### 問題 3: 環境依存

ユーザーの環境はそれぞれ異なります。既存のインストールがあるか、依存関係は満たされているか、などを考慮する必要があります。

## PAI の解決策：ウィザード形式 AI インストール

PAI は **5フェーズのウィザード形式** でこれらの問題を解決しています。
...
**重要なポイント:** `src/` ディレクトリには **完全なコードファイル** が入っています。マークダウンからコードを抽出するのではなく、実ファイルをそのままコピーします。これにより AI による「簡略化」を防げます。

## Phase 1: システム分析

AI はまずユーザーの環境をチェックします。`INSTALL.md` には、実行すべきコマンドが書かれています：
...
**重要:** 必須の依存関係がない場合は、ここでインストールを中断します。

## Phase 2: ユーザーへの質問

Claude Code の **AskUserQuestion** ツールを使って、対話形式で選択肢を提示します：
...
**条件分岐がポイント:** 問題がなければ質問をスキップします。すべての質問を毎回聞くのではなく、必要な質問だけを行います。

### 質問の設計ガイドライン

PAI では質問の設計にもルールがあります：
...
タイムスタンプ付きなので、複数回インストールしても過去のバックアップが上書きされません。

## Phase 4: インストール実行

Claude Code の **TodoWrite** ツールで進捗を可視化しながら実行します：
...
**ポイント:** `src/` ディレクトリから **そのままコピー** します。AI がコードを解釈・変換する必要がないため、完全なコードが確実にインストールされます。

## Phase 5: 検証

`VERIFY.md` に書かれたチェックリストを実行します：
...
すべてのチェックが通れば成功メッセージ、失敗があればトラブルシューティングの案内を表示します。
...
PAI パックには重要な原則があります：**End-to-End で完全であること**。
...
これにより、「インストールしたけど動かない」という事態を防げます。
...
PAI の AI インストールは「AI がツールをセットアップする」という新しいパラダイムを示しています。

- 人間は「これをインストールして」と頼むだけ
- AI が環境を分析し、対話形式で進め、検証まで行う

この仕組みは、AI エージェントのスキルやプラグインを配布する際の参考になるでしょう。
